...
class Menu_LoadJson(tk.Frame):
    ...
    def load_json(self):
        ...
        if path:
            ...
            with open(path) as json_file:
                ...
                self.load_sprite_sheet(json_data)
    ...
    def load_sprite_sheet(self, json_data): # add a bunch of checks for these images; do the files actually exist? etc.
        header = json_data["header"]
        # output_folder_path = header["output_folder_path"]
        layer_info = header["layer_info"]
        pose_objects = json_data["pose_objects"]
        images = json_data["images"]

        border_image = Image.open(header["border_image_path"])

        with Image.open(header["border_image_path"]) as border_image:
            sprite_sheet = border_image.copy()
            # border_image.close()

        # color_transparent = ImageColor.getrgb("#00000000")

        for i in range(len(layer_info) - 1, -1, -1): # WHY does it need to go to -1. that feels bad
            ...
            for pose in pose_objects:
                ...
                for limb in limb_objects:
                    if limb["name"] == curr_layer_name: # does not work for images wherein different layers have the same name. either support this or prevent entirely
                        ...
                        with Image.open(images[limb["image_index"]]) as limb_image:
            ...
            sprite_sheet = Image.alpha_composite(sprite_sheet, layer_image)
        
        # sprite_sheet.show()

        sprite_sheet_path = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("Image Files", "*.png;*.jpg;*.jpeg")])
